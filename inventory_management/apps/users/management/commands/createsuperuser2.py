...
from inventory_management.apps.users.tasks import credential_mail_task
        ...
        try:
            ...
            while True:
                ...
                if branch_id.isdigit():
                    ...
                    not_unique = False
                    for shop in shops:
                        if shop.branch_id == branch_id:
                            not_unique = True
                    if not_unique:
                        self.stderr.write("Error: You must use unique branch id")
                        continue
                    break
                else:
                    self.stderr.write("Error: Please Use Numbers")
                    continue
            ...
            while True:
                ...
                if phone.isdigit():
                    if len(str(phone)) != 10:
                        self.stderr.write("Error: Phone must be a 10 Digit number")
                        continue
                    phone = int(phone)
                    break
                else:
            ...
            shop = Shop.objects.create(name=shop_name, branch_id=branch_id)
            user_data['shop'] = shop
            user_data['phone'] = phone

            # sending mail with celery
            activation_link = "http://127.0.0.1:8000"
            credential_mail_task.delay(activation_link, user_data['email'], user_data['password'])
